...
class AdsManager {
    ...
    fun initialize(context: Context) {
        ...
        setEnabledNoAds(SharedPrefHelper.readBoolean(Constants.ADS_ID_KEY))
...
//        loadInterstitial()
    ...
    fun showInterstitialAd(activity: Activity, onAdLoaded: OnAdLoaded) {
        if (isEnabledNoAds) {
            return
        }
        if (mInterstitialAd != null) {
            AdsLoadingDialog(activity, mInterstitialAd)
            if (mInterstitialAd == null) {
                return
            }
...
//            loadInterstitial()
    ...
    fun showAdMobBanner(activity: Activity, frameLayout: FrameLayout?, onAdLoaded: OnAdLoaded) {
        if (!isNetworkAvailable(context) || isEnabledNoAds) {
            onAdLoaded.OnAdLoadedCallBack(false)
